    ...
    <form ref={formRef} action={action} className="space-y-3">
      <div className="grid grid-cols-[2fr_1fr_1fr] gap-2">
        <label className="block">
          ...
          <Input name="name" required placeholder={t("session.add.namePlaceholder")} />
        </label>
        <label className="block">
          <Label>{t("session.add.init")}</Label>
          <Input name="initiative" type="number" required min={-10} max={50} />
        </label>
        <label className="block">
          <Label>HP</Label>
          <Input name="maxHp" type="number" min={0} max={9999} placeholder="—" />
        </label>
      </div>
